...
export default function OtherMotor() {
  ...
  const userAds = useUserAds(['Other']);
      ...
      <div className="max-w-7xl mx-auto px-4 py-4">
        ...
        <div className="mb-5">
          <div className="flex items-center gap-8 mb-3">
            <h1 className="text-2xl font-bold text-foreground whitespace-nowrap">Other items</h1>
          </div>
        </div>
        ...
        <MobileCategoryFilters search={search} onSearchChange={(e) => setSearch(e.target.value)} placeholder="Search Other items">
          <SimpleFiltersSidebar dealerLabel="Trader" onFilterChange={(f) => { setActiveFilters(f); setCurrentPage(1); }} />
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1 min-w-0 min-h-[calc(100vh+4rem)]">
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm text-muted-foreground"><span className="font-semibold text-foreground">{allFiltered.length.toLocaleString()}</span> ads for Other items in Ireland</p>
