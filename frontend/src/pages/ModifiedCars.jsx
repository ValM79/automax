...
export default function ModifiedCars() {
  ...
  const userAds = useUserAds(['Modified Cars']);
  ...
  const allFiltered = allListings.filter((item) => {
    ...
    const { vehicles, yearFrom, yearTo, priceFrom, priceTo, county } = activeFilters;
      ...
      <div className="max-w-7xl mx-auto px-4 py-4">
        ...
        <div className="mb-5">
          <div className="flex items-center gap-8 mb-3">
            <h1 className="text-2xl font-bold text-foreground whitespace-nowrap">Modified Cars</h1>
          </div>
        </div>

        <div className="mb-6 rounded-xl overflow-hidden h-36 sm:h-44 bg-card">
          <img src="/img/7b875ae82_generated_image.jpg" alt="Promo" className="w-full h-full object-cover" />
        </div>

        {/* Mobile: search + help text + filters */}
        <MobileCategoryFilters search={search} onSearchChange={(e) => setSearch(e.target.value)} placeholder="Search Modified Cars">
          ...
          <div className="flex-1 min-w-0 min-h-[calc(100vh+4rem)]">
            ...
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm text-muted-foreground">
                <span className="font-semibold text-foreground">{allFiltered.length.toLocaleString()}</span> ads for Modified Cars in Ireland
              </p>
