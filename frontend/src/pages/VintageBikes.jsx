...
export default function VintageBikes() {
  ...
  const userAds = useUserAds(['Vintage Bikes']);
      ...
      <div className="max-w-7xl mx-auto px-4 py-4">
        ...
        <div className="mb-5">
          <div className="flex items-center gap-8 mb-3">
            <h1 className="text-2xl font-bold text-foreground whitespace-nowrap">Vintage Bikes</h1>
          </div>
        </div>
        <div className="mb-6 rounded-xl overflow-hidden border border-border h-36 sm:h-44 bg-card">
          <img src="/img/78a162d71_generated_image.jpg" alt="Vintage Bikes Banner" className="w-full h-full object-cover" />
        </div>
        {/* Mobile: search + help text + filters */}
        <MobileCategoryFilters search={search} onSearchChange={(e) => setSearch(e.target.value)} placeholder="Search Vintage Bikes">
          <FiltersSidebar alwaysOpen onFilterChange={(f) => { setActiveFilters(f); setCurrentPage(1); }} />
        ...
        <div className="hidden lg:flex items-center gap-8 mb-5">
          <div className="relative flex-1 max-w-[35%]">
            ...
            <input
              ...
              placeholder="Search Vintage Bikes"
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1 min-w-0 min-h-[calc(100vh+4rem)]">
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm text-muted-foreground"><span className="font-semibold text-foreground">{allFiltered.length.toLocaleString()}</span> ads for Vintage Bikes in Ireland</p>
